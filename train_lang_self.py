...
lang_opt_map = {"en": "en-us", "fr": "fr-fr", "es": "es", "it": "it", "de": "de", "ru": "ru",
             "english": "en-us", "french": "fr-fr", "spanish": "es", "italian": "it", "german": "de"}
lang_encode = {"en-us": 1, "fr-fr": 2, "es": 3, "it": 4, "de": 5, "ru": 6}
...
class ASR(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        tokens_lang, tokens_lang_len = batch.token_lang
        ...
        enc_out, pred, p_lang = self.modules.Transformer(
            src, tokens_bos, wav_lens, pad_idx=self.hparams.pad_index, condition_emb=None
        )
        ...
        return p_ctc, p_seq, p_lang, wav_lens, hyps

    def compute_objectives(self, predictions, batch, stage):
        """Computes the loss (CTC+NLL) given predictions and targets."""

        (p_ctc, p_seq, p_lang, wav_lens, hyps,) = predictions
        ...
        loss_lang = self.hparams.lang_cost(
            p_lang, tokens_lang[:, 0]
        ).sum()

        loss = (
                self.hparams.ctc_weight * loss_ctc
                + (1 - self.hparams.ctc_weight) * loss_seq
                + self.hparams.lang_weight * loss_lang
        )
        ...
        if stage != sb.Stage.TRAIN:
            ...
            pred_lang = torch.argmax(p_lang, dim=1)
            ...
            self.lang_metrics.append(ids, p_lang.unsqueeze(1), tokens_lang, tokens_lang_len)

            # self.interm_lang.append(p_lang.unsqueeze(1), tokens_lang, tokens_lang_len)
            pred_lang = [str(p) for p in pred_lang.tolist()]
            tokens_lang = [str(p) for p in tokens_lang.squeeze(1).tolist()]
            self.interm_lang.append(ids, pred_lang, tokens_lang, tokens_lang)
    ...
    def on_stage_start(self, stage, epoch):
        ...
        self.interm_lang = self.hparams.acc_lang_computer()
        self.seq_metrics = self.hparams.seq_stats()
        self.lang_metrics = self.hparams.lang_stats()
        if stage != sb.Stage.TRAIN:
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            stage_stats["LANG_ACC"] = self.interm_lang.summarize()['accuracy']
            current_epoch = self.hparams.epoch_counter.current
            valid_search_interval = self.hparams.valid_search_interval
        ...
        if stage == sb.Stage.VALID and sb.utils.distributed.if_main_process():
            ...
            epoch_stats = {
                ...
                "lang_loss": self.lang_metrics.summarize("average"),
            }
            ...
            with open(self.hparams.wer_file.replace("wer", "acc"), "w") as w:
                self.interm_lang.write_stats(w)
...
def dataio_prepare(hparams):
    ...
    @sb.utils.data_pipeline.takes("wrd", "spk_id")
    @sb.utils.data_pipeline.provides(
        "wrd", "tokens_list", "token_lang", "tokens_bos", "tokens_eos", "tokens"
    )
    def text_pipeline(wrd, spk_id):
        yield wrd
        tokens_list = label_encoder.encode_sequence(wrd)
        yield tokens_list
        token_lang = torch.LongTensor([lang_encode[lang_opt_map[spk_id.split("_")[-1]]] - 1])
        yield token_lang
    ...
    sb.dataio.dataset.set_output_keys(
        datasets, ["id", "sig", "wrd", "token_lang", "tokens_bos", "tokens_eos", "tokens"],
    )
