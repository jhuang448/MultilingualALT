...
Override for language self-conditioning experiments
"""
...
class MyTransformerASR(MyTransformerInterface):
    ...
    def __init__(
        ...
        lang_dim=-1,
        condition_dim=0,
        encoder_condition=False,
        decoder_condition=False,
    ):
        super().__init__(
            tgt_vocab=tgt_vocab,
            d_model=d_model,
        ...
        self.lang_lin = Linear(input_size=d_model, n_neurons=lang_dim)
        self.emb = Linear(input_size=lang_dim, n_neurons=condition_dim)

        if decoder_condition:
            self.custom_enc_module = ModuleList(
                Linear(
                    input_size=d_model+condition_dim,
                    n_neurons=d_model,
                    bias=True,
                    combine_dims=False,
                ),
    ...
    def forward(self, src, tgt, wav_len=None, pad_idx=0, condition_emb=None):
        """
        Arguments
        ----------
        src : torch.Tensor
        ...
        if self.encoder_condition:
            src = torch.concat((src, condition_emb), dim=2)
        ...
        # predict language
        lang_mean = torch.mean(encoder_out, dim=1)
        p_lang = F.softmax(self.lang_lin(lang_mean), dim=1)
        condition_emb = self.emb(p_lang).unsqueeze(1)
        condition_emb = condition_emb.repeat(1, encoder_out.shape[1], 1)
        ...
        if self.decoder_condition:
            encoder_out = torch.concat((encoder_out, condition_emb), dim=2)
            encoder_out = self.custom_enc_module(encoder_out)
        ...
        return encoder_out, decoder_out, p_lang
